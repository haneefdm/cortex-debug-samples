...
export class AddressRangesInUse {
    ...
    public static doLog(
        stream: WriteStream | NodeJS.WriteStream, prefix: string, base: number, len: number, ranges: AddrRange[]): void {
        stream.write(prefix + ` base=0x${base.toString(16)}, totalLen=${len}, #ranges=${ranges.length}\n`);
        let bc = 0;
        for (const range of ranges) {
            bc += range.length;
            stream.write(`**** 0x${range.base.toString(16)}, len=${range.length}, cum-bytes=${bc}\n`);
        }
        const diff = len - bc;
        if ((bc > 0) && (len > 0)) {
            const percent = (diff / len) * 100;
            stream.write(prefix + ` totalLen=${len}, savings=${diff} ${percent.toFixed(2)}%\n`);
        }
    }
}
